let paypalColor = "green"

$(function () {
    window.addEventListener('message', function (event) {
        if (event.data.type === "openpaypal") {
            $('#paypalui').fadeTo(10, 1.0)
            $('#depositval').val('');
            $('#withdrawval').val('');
...
$('.close').click(function () {
    $('paypalui').hide();
    removeColors(paypalColor)
    $.post('http://r5m-paypal/NUIFocusOff', JSON.stringify({}));
});

$('#withdraw').click(function (e) {
    e.preventDefault();
    $.post('http://r5m-paypal/withdrawl', JSON.stringify({
...
$('#deposit').click(function (e) {
    e.preventDefault();
    $.post('http://r5m-paypal/deposit', JSON.stringify({
...
$('#transfer').click(function (e) {
    e.preventDefault();
    $.post('http://r5m-paypal/transfer', JSON.stringify({
        to: $('#idval').val(),
